...
* above the GRS80 reference ellipsoid ( ae = 6378137.0 meter; f = 1/298.257 )
...
*------------------------------------------------------------------------------
      real*8   	rev
      integer*4	maxnrarcs
      parameter	(maxnrarcs=1599)
      integer*4	istart,iend,iskip(maxnrarcs),iarc,iskip0,iskip1,
     |	itime0,itime1,itstep,getorbfile,odrinfo,mdate,lnblnk,
     |	i,j,l,jarc,irep,nrec,remid,maxarc,minarc,nrev,unit,freeunit,
     |  yymmdd0,hh0,mm0,yymmdd1,hh1,mm1,yymmdd2,hh2,mm2,ss2
      logical*4	exist(0:maxnrarcs)
      character	satel*8,odrnm*80,old_path*80,line*80
...
* The first skip may be set to the beginning of the first arc
	    ...
	    if (iarc.eq.minarc) iskip(iarc)=itime0
	    goto 10
11	    close (unit)
	    if (verbose) write (0,611)
	 else

* No arclist. Scan directory.

	    if (verbose) write (0,609) odrnm(:l)
...
* - Load new orbit
	 ...
	 do iarc=minarc,maxarc
	    if (time.ge.iskip(iarc) .and. time.le.iskip(iarc+1) .and.
     |	  exist(iarc)) goto 15
	 enddo
	 getorb=2
	 return
...
* Call GETORBFILE
  ...
  611 format (')')
  620 format (z1,i2,2x,i6,1x,i2,1x,i2, 3x,i6,1x,i2,1x,i2,
...
* Open next orbit data file
	 ...
	 old_file=file
	 l=lnblnk(file)
	 if (verbose) write (0,610) file(1:l)
...
* they are in 10^-7 degrees.
	 ...
	 if (type.lt.0) then
	    ...
	    close(unit)
	    write (0,550) 'getorb: ODR file too big'
	    getorbfile=1
	    return
	 endif
         if (type.eq.0) then
...
* Check time limits
      ...
      if (time.ge.istart .and. time.le.iend) then
         getorbfile=0
      else if (time.lt.itime0 .or. time.gt.itime1) then
	 getorbfile=2
	 return
      else
	 getorbfile=-1
